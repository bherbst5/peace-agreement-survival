...
Time-varying terms (PH violators)
----------------------------------
Variables that fail the PH assumption in the baseline Cox model are entered
as  x + x*log(t).  The interaction column x*log(t) is RESIDUALIZED against
x before entering the lasso.  Without residualization, if the lasso zeros x
while retaining x*log(t), the interaction coefficient conflates the main
effect with the time-varying component and becomes uninterpretable.
Residualizing makes the two columns orthogonal so each can be selected or
zeroed independently.

NOTE ON TIME CONSTRUCTION FOR TV TERMS
The fully correct approach (used in baseline_cox_full.py) evaluates log(t_k)
at each failure time t_k for every at-risk observation, via a person-period
expansion.  scikit-survival's CoxnetSurvivalAnalysis does not support
start-stop (left-truncated) format, so here we use each subject's own
observed time log(t_i + 1) as the interaction value.  This is an
approximation; the residualization ensures the interpretability problem the
advisor identified is still resolved even under this approximation.
...
#     Stable, tie-safe Breslow gradient for Cox lasso path setup.
# ═══════════════════════════════════════════════════════════════════════════════
def _grad_at_zero(X, t, e):
    """Gradient of negative partial log-likelihood at beta=0 (Breslow)."""
    order = np.argsort(t, kind="mergesort")  # stable sort for ties
    X_s = X[order]
    t_s = t[order]
    e_s = e[order].astype(bool)

    n_ev = int(e_s.sum())
    grad = np.zeros(X.shape[1], dtype=float)

    # Breslow tie handling: group by event time
    for tt in np.unique(t_s[e_s]):
        event_mask = (t_s == tt) & e_s
        risk_mask = t_s >= tt

        d = int(event_mask.sum())
        grad += X_s[event_mask].sum(axis=0) - d * X_s[risk_mask].mean(axis=0)
    
    return grad

# ═══════════════════════════════════════════════════════════════════════════════
# 4.  λ GRID + SOLUTION PATH (Auto-scaled by scikit-survival)
# ═══════════════════════════════════════════════════════════════════════════════
...
for k, (train_idx, val_idx) in enumerate(kf.split(X_std)):
    ...
    for j in range(len(lam_grid)):
        ...
        # ── THE VERWEIJ FIX (glmnet standard) ────────────────────────────────
        # Calculating Cox LL directly on tiny validation folds is biased against
        # non-zero models due to rapidly shrinking risk sets. We compute the 
        # full data LL and subtract the training data LL to get the validation LL.
        #
        # Because partial_loglik is scaled by 1/n_events, we multiply by the 
        # event count to get the raw log-likelihood, subtract, then divide by 
        # the validation event count.
